...
export function BrainCenterpiece({
  ...
  if (fill) {
    return <NeuralBrain state={state} activeRegions={activeRegions} className="absolute inset-0" />
  }

  const active = state !== 'idle'
  return (
    <div className={`relative mx-auto aspect-square w-full ${compact ? 'max-w-[8.5rem]' : 'max-w-[34rem]'}`}>
      <NeuralBrain state={state} activeRegions={activeRegions} className="absolute inset-0" />
